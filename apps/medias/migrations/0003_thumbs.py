...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for media in Media.objects.all().select_subclasses():
            
            if media.is_an_album:
                try:
                    thumb = Thumbnail.objects.get(media_id=media.get_children().models(Image, Video)[0], size='small')
                    setattr(media, 'url_%s' % thumb.size, thumb.url)
                    setattr(media, 'height_%s' % thumb.size, thumb.height)
                    setattr(media, 'width_%s' % thumb.size, thumb.width)
                except Exception,e:
                    print e
            else:
                for thumb in Thumbnail.objects.filter(media=media):
                    setattr(media, 'url_%s' % thumb.size, thumb.url)
                    setattr(media, 'height_%s' % thumb.size, thumb.height)
                    setattr(media, 'width_%s' % thumb.size, thumb.width)
