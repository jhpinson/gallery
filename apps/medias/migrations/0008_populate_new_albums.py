...
from structures.models import Album as OldAlbum
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for old in OldAlbum.objects.all().order_by('lft'):
            
            a = Album()
            
            a.created_by_id = old.created_by_id if old.created_by is not None else 1
            a.modified_by_id = old.modified_by_id if old.modified_by is not None else 1
            a.created_at = old.created_at
            a.modified_at = old.modified_at
            a.date = old.start_date
            a.end_date = old.end_date
            a.name = old.name
            #a.album_id = None if old.parent is None else old.parent_id
            a.old_id = old.pk
            a.oldalbum = old.parent_id
