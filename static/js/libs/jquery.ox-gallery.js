(function($) {
  ...
  var initGallery = function(items) {
    ...
    var getPrefix = function getPrefix(prop) {
      var prefixes = ['Moz', 'Khtml', 'Webkit', 'O', 'ms'], elem = document
          .createElement('div'), upper = prop.charAt(0).toUpperCase()
          + prop.slice(1);
    ...
    var ismouseover = function(overThis, x, y) {
      ...
      result = offset.left <= x
          && offset.left + $(overThis).outerWidth() > x
          && offset.top <= y
          && offset.top + $(overThis).outerHeight() > y;
      return result;
    };
    ...
    var syncLegendVisibility = function(force) {
      ...
      var _displayLegend = ismouseover(currentSlide.find('img:first'),
          lastMousePos.x, lastMousePos.y);
      displayLegend = _displayLegend;
      if (displayLegend
          && currentSlide.find('.legend:visible').length == 0) {
        currentSlide.find('.legend').fadeIn(200);
      } else if (!displayLegend
          && currentSlide.find('.legend:visible').length == 1) {
    ...
    var createEffectStyle = function() {
      ...
      var effect = [
          ...
          '60% { ' + prefix + 'transform: translateX(-'
              + (offset + 30) + 'px);}',
          '80% { ' + prefix + 'transform: translateX(-'
              + (offset - 10) + 'px);}',
          '100% { ' + prefix + 'transform: translateX(-' + offset
              + 'px);}',
          '}',
      ...
      var changeClass = [
          '.transition-next {',
          prefixAnimation + 'animation: ' + effectDuration
              + 'ms ease;',
      ...
      if (!document.getElementById('glisse-css')) {
        $('<style type="text/css" id="glisse-css">' + effect
            + changeClass + '</style>').appendTo('head');
      } else {
    ...
    var preload = function(callback) {
      ...
      var imagesLength = items.length;
      var callbackCalled = false;
      function _preloadImage(idx) {
    ...
    var displaySlide = function(pos, next) {
      ...
      slide.find('img').load(function() {
        ...
        if (supportEffects) {
          var transitionClass = next
              ? 'transition-next'
              : 'transition-prev';
          slideContainer.children('.item').addClass(transitionClass);
          setTimeout(function() {
                if (slideContainer.children('.item').length == 2) {
                  slideContainer.children('.item:first')
                      .remove();
                }
                slide.css('left', '0px');
          ...
          slideContainer.children('.item').css('left',
              (next ? '-=' : '+=') + offset + 'px');
          if (slideContainer.children('.item').length == 2) {
            slideContainer.children('.item:first').remove();
          }
